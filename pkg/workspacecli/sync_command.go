...
func planForgeWorkspaces(ctx context.Context, cfg *config.Config, out io.Writer, strategyOverrideStr string, fullOutput bool) ([]reposync.Action, error) {
	...
	for name, ws := range workspaces {
		...
		// Fallback to root config's active profile
		...
		// Fallback to root config direct values
		...
		if ws.Sync != nil && ws.Sync.CleanupOrphans {
			planReq.Options.CleanupOrphans = true
			planReq.Options.Roots = []string{wsPath}
		}
...
func ensureChildConfigs(out io.Writer, cfg *config.Config) error {
	...
	for name, ws := range cfg.Workspaces {
		...
		// Use Force variant to replace auto-generated bootstrap files from previous runs.
		if ws.ConfigLink != "" {
			childConfigFile := filepath.Join(wsPath, ".gz-git.yaml")
			isSymlink, _ := config.IsConfigSymlink(childConfigFile)
			if !isSymlink {
				// Check if existing file is auto-generated (safe to replace)
				if content, err := os.ReadFile(childConfigFile); err == nil {
					...
					if !isGenerated {
						return fmt.Errorf("workspace '%s': config file %s exists and is user-maintained; remove it manually to use configLink", name, childConfigFile)
					}
				}
			}
