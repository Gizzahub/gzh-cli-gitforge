...
func ensureChildConfigs(out io.Writer, cfg *config.Config) error {
	...
	for name, ws := range cfg.Workspaces {
		...
		if _, err := os.Stat(childConfigFile); os.IsNotExist(err) {
			...
			// We inherit profile from this workspace definition or current
			profileName := ws.Profile
			if profileName == "" {
				// If not set on workspace, it might be inherited from parent logic
				// But usually we want to explicit set it if different.
				// If empty, child will naturally inherit from parent lookup anyway.
				// So we don't strictly need to write it unless we want to lock it down.
				// Let's leave it empty to respect dynamic lookup.
			}
			...
			if err := os.WriteFile(childConfigFile, []byte(content), 0644); err != nil {
