...
var (
	fetchBulkFlags      BulkCommandFlags
	fetchBulkAllRemotes bool
	fetchBulkPrune      bool
	fetchBulkTags       bool
)

// fetchBulkCmd represents the fetch-bulk command
var fetchBulkCmd = &cobra.Command{
	Use:   "fetch-bulk [directory]",
	Short: "Fetch updates from remote repositories in bulk",
...
It only updates remote-tracking branches.`,
	Example: `  # Fetch all repositories in current directory (1-level scan)
  gz-git fetch-bulk --scan-depth 1

  # Fetch all repositories up to 2 levels deep
  gz-git fetch-bulk -d 2 .

  # Fetch with custom parallelism
  gz-git fetch-bulk --parallel 10 ~/projects

  # Fetch from all remotes
  gz-git fetch-bulk --all-remotes ~/workspace

  # Fetch and prune deleted remote branches
  gz-git fetch-bulk --prune ~/projects

  # Fetch all tags
  gz-git fetch-bulk --tags ~/repos

  # Dry run to see what would be fetched
  gz-git fetch-bulk --dry-run ~/projects

  # Filter by pattern
  gz-git fetch-bulk --include "myproject.*" ~/workspace

  # Exclude pattern
  gz-git fetch-bulk --exclude "test.*" ~/projects

  # Compact output format
  gz-git fetch-bulk --format compact ~/projects

  # Continuously fetch at intervals (watch mode)
  gz-git fetch-bulk --scan-depth 2 --watch --interval 5m ~/projects

  # Watch with shorter interval
  gz-git fetch-bulk --watch --interval 1m ~/work`,
	Args: cobra.MaximumNArgs(1),
	RunE: runFetchBulk,
}

func init() {
	rootCmd.AddCommand(fetchBulkCmd)

	// Common bulk operation flags
	addBulkFlags(fetchBulkCmd, &fetchBulkFlags)

	// Fetch-specific flags
	fetchBulkCmd.Flags().BoolVar(&fetchBulkAllRemotes, "all-remotes", false, "fetch from all remotes (not just origin)")
	fetchBulkCmd.Flags().BoolVarP(&fetchBulkPrune, "prune", "p", false, "prune remote-tracking branches that no longer exist")
	fetchBulkCmd.Flags().BoolVarP(&fetchBulkTags, "tags", "t", false, "fetch all tags from remote")
}

func runFetchBulk(cmd *cobra.Command, args []string) error {
	...
	// Validate depth
	if err := validateBulkDepth(cmd, fetchBulkFlags.Depth); err != nil {
		return err
	}

	// Validate format
	if err := validateBulkFormat(fetchBulkFlags.Format); err != nil {
		return err
	}
	...
	opts := repository.BulkFetchOptions{
		Directory:         directory,
		Parallel:          fetchBulkFlags.Parallel,
		MaxDepth:          fetchBulkFlags.Depth,
		DryRun:            fetchBulkFlags.DryRun,
		Verbose:           verbose,
		AllRemotes:        fetchBulkAllRemotes,
		Prune:             fetchBulkPrune,
		Tags:              fetchBulkTags,
		IncludeSubmodules: fetchBulkFlags.IncludeSubmodules,
		IncludePattern:    fetchBulkFlags.Include,
		ExcludePattern:    fetchBulkFlags.Exclude,
		Logger:            logger,
		ProgressCallback:  createProgressCallback("Fetching", fetchBulkFlags.Format, quiet),
	}

	// Watch mode: continuously fetch at intervals
	if fetchBulkFlags.Watch {
		return runFetchBulkWatch(ctx, client, opts)
	}

	// One-time fetch
	if shouldShowProgress(fetchBulkFlags.Format, quiet) {
		fmt.Printf("Scanning for repositories in %s (depth: %d)...\n", directory, fetchBulkFlags.Depth)
	}
	...
	if shouldShowProgress(fetchBulkFlags.Format, quiet) && result.TotalScanned == 0 {
		fmt.Printf("Scan complete: no repositories found\n")
	}

	// Display results (always output for JSON format, otherwise respect quiet flag)
	if fetchBulkFlags.Format == "json" || !quiet {
		displayFetchBulkResults(result)
	}

	return nil
}

func runFetchBulkWatch(ctx context.Context, client repository.Client, opts repository.BulkFetchOptions) error {
	if !quiet {
		fmt.Printf("Starting watch mode: fetching every %s\n", fetchBulkFlags.Interval)
	...
	ticker := time.NewTicker(fetchBulkFlags.Interval)
	defer ticker.Stop()

	// Perform initial fetch immediately
	if err := executeFetchBulk(ctx, client, opts); err != nil {
		return err
	}
...
func executeFetchBulk(ctx context.Context, client repository.Client, opts repository.BulkFetchOptions) error {
	result, err := client.BulkFetch(ctx, opts)
	if err != nil {
		return fmt.Errorf("bulk fetch failed: %w", err)
	}
...
func displayFetchBulkResults(result *repository.BulkFetchResult) {
	// JSON output mode
	if fetchBulkFlags.Format == "json" {
		displayFetchBulkResultsJSON(result)
		return
	}

	// LLM output mode
	if fetchBulkFlags.Format == "llm" {
		displayFetchBulkResultsLLM(result)
		return
	}
	...
	if fetchBulkFlags.Format != "compact" && len(result.Repositories) > 0 {
		fmt.Println("Repository details:")
		for _, repo := range result.Repositories {
			displayFetchBulkRepositoryResult(repo)
		}
	}

	// Display only errors/warnings in compact mode
	if fetchBulkFlags.Format == "compact" {
		hasIssues := false
		for _, repo := range result.Repositories {
...
func displayFetchBulkRepositoryResult(repo repository.RepositoryFetchResult) {
...
// FetchBulkJSONOutput represents the JSON output structure for fetch-bulk command
type FetchBulkJSONOutput struct {
	TotalScanned   int                             `json:"total_scanned"`
	TotalProcessed int                             `json:"total_processed"`
	DurationMs     int64                           `json:"duration_ms"`
	Summary        map[string]int                  `json:"summary"`
	Repositories   []FetchBulkRepositoryJSONOutput `json:"repositories"`
}

// FetchBulkRepositoryJSONOutput represents a single repository in JSON output
type FetchBulkRepositoryJSONOutput struct {
...
func displayFetchBulkResultsJSON(result *repository.BulkFetchResult) {
	output := FetchBulkJSONOutput{
		TotalScanned:   result.TotalScanned,
		TotalProcessed: result.TotalProcessed,
		DurationMs:     result.Duration.Milliseconds(),
		Summary:        result.Summary,
		Repositories:   make([]FetchBulkRepositoryJSONOutput, 0, len(result.Repositories)),
	}

	for _, repo := range result.Repositories {
		repoOutput := FetchBulkRepositoryJSONOutput{
...
func displayFetchBulkResultsLLM(result *repository.BulkFetchResult) {
	output := FetchBulkJSONOutput{
		TotalScanned:   result.TotalScanned,
		TotalProcessed: result.TotalProcessed,
		DurationMs:     result.Duration.Milliseconds(),
